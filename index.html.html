...
.tabs{display:flex;overflow-x:100;scrollbar-width:none;border-bottom:1px solid var(--border);background:var(--bg2);padding:0 16px}
  ...
  <div class="login-wrap">
    ...
    <div class="how-to-use-box">
      ...
      <div class="how-to-use-step">
        ...
        <div class="step-text">المدير: يسجل دخول بكلمة السر - مرتبط بجهازين فقط (هاتف + آيباد)</div>
      </div>
      <div class="how-to-use-step">
  ...
  <div class="login-wrap">
    ...
    <div class="card">
      ...
      <div id="login-admin">
        ...
        <p style="color:var(--muted);font-size:12px;margin-top:8px;text-align:center">⚠️ حساب المدير مرتبط بجهازين فقط (هاتف + آيباد)</p>
      </div>
...
<!-- باقي الأقسام (ADMIN SCREEN, CUSTOMER SCREEN, MODALS) - نفس الكود السابق -->
<!-- للاختصار، تم حذف HTML الأقسام لأنها نفسها بالضبط، والتركيز على JavaScript -->

<script>
// ==================== CORE FUNCTIONS ====================
const ENCRYPTION_KEY = "S@yr4f4#M$st4qb4l!2025*XK9@zQ7!#&^mP3rVnL8wE";
const STORE = 'sarafa_final_v5';
...
function initDB() {
  return {
    ...
    admin: { 
      ...
      allowedDevices: [] // ✅ قائمة الأجهزة المسموحة (هاتف + آيباد)
    },
    dailyLedger: {},
...
function load() {
  try {
    ...
    if (encrypted) {
      ...
      if (!data.admin) data.admin = { email: '', passwordHash: ADMIN_HASH, allowedDevices: [] };
      return data;
    }
  } catch(e) {}
  return initDB();
...
// ✅✅✅ تسجيل دخول المدير - مرتبط بجهازين فقط (هاتف + آيباد) ✅✅✅
function loginAdmin() {
  const pass = el('admin-pass').value;
  const admin = DB.admin || { email: '', passwordHash: ADMIN_HASH, allowedDevices: [] };
  const currentDeviceId = getDeviceId();
  const deviceType = isIPad() ? 'iPad' : 'Phone';
  ...
  // إذا لم تكن هناك أجهزة مسموحة بعد (أول تسجيل دخول)
  if (!admin.allowedDevices || admin.allowedDevices.length === 0) {
    admin.allowedDevices = [currentDeviceId];
    if (!DB.admin) DB.admin = admin;
    save(DB);
    alert('✅ تم ربط حساب المدير بهذا الجهاز (' + deviceType + '). يمكنك إضافة آيباد أيضاً.');
  }
  
  // إذا كان هناك جهاز واحد فقط مسموح، نسمح بإضافة جهاز ثاني (آيباد)
  if (admin.allowedDevices.length === 1 && !admin.allowedDevices.includes(currentDeviceId)) {
    // نسمح بإضافة الجهاز الثاني فقط إذا كان آيباد أو إذا كان الجهاز الأول آيباد والثاني هاتف
    const firstDeviceIsIPad = admin.allowedDevices[0].includes('iPad') || false;
    const currentIsIPad = isIPad();
    
    if ((firstDeviceIsIPad && !currentIsIPad) || (!firstDeviceIsIPad && currentIsIPad)) {
      // جهازين مختلفين (هاتف + آيباد) - نسمح
      admin.allowedDevices.push(currentDeviceId);
      save(DB);
      alert('✅ تم إضافة ' + deviceType + ' كجهاز ثاني مسموح به.');
    } else {
      alert('❌ لا يمكن الدخول من هذا الجهاز!\n\nمسموح فقط بجهازين: هاتف + آيباد.');
      return;
    }
  }
  
  // التحقق من أن الجهاز الحالي ضمن الأجهزة المسموحة
  if (!admin.allowedDevices.includes(currentDeviceId)) {
    alert('❌ لا يمكن الدخول من هذا الجهاز!\n\nالأجهزة المسموحة: ' + admin.allowedDevices.length + ' جهاز(ة) فقط.');
    return;
  }
...
// ==================== ADMIN PROFILE (إدارة الأجهزة المسموحة) ====================
function showAdminProfileModal() { 
  el('modal-admin-profile').classList.add('show'); 
  updateAllowedDevicesList();
}

function updateAllowedDevicesList() {
  const admin = DB.admin || { allowedDevices: [] };
  const container = el('allowed-devices-list');
  if (!container) return;
  
  if (admin.allowedDevices.length === 0) {
    container.innerHTML = '<p style="color:var(--muted)">لا توجد أجهزة مسجلة بعد</p>';
    return;
  }
  
  container.innerHTML = admin.allowedDevices.map((dev, i) => {
    const isIPad = dev.includes('iPad') || false;
    return `<div style="display:flex;justify-content:space-between;padding:8px;background:var(--bg3);border-radius:8px;margin-bottom:8px">
      <span>📱 ${isIPad ? 'آيباد' : 'هاتف'} (${dev.substring(0, 15)}...)</span>
      <button onclick="removeAllowedDevice(${i})" style="background:var(--red);border:none;color:white;padding:4px 8px;border-radius:4px;cursor:pointer">❌</button>
    </div>`;
  }).join('');
}

function removeAllowedDevice(index) {
  if (!confirm('هل أنت متأكد من حذف هذا الجهاز؟')) return;
  DB.admin.allowedDevices.splice(index, 1);
  save(DB);
  updateAllowedDevicesList();
}
...
<div class="modal-overlay" id="modal-admin-profile">
  <div class="modal">
    ...
    <label>📱 الأجهزة المسموحة (حد أقصى 2: هاتف + آيباد)</label>
    <div id="allowed-devices-list" style="margin-bottom:16px"></div>
    <p style="color:var(--muted);font-size:12px;text-align:center">⚠️ يمكنك حذف جهاز لإضافة جهاز جديد</p>
  </div>
</div>
